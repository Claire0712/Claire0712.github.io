...
export const timelineMarkup = (records, locale) => records.map((item) => {
  ...
  if (isCard) {
    ...
    const supervisor = item.supervisor?.url?.startsWith('https://')
      ? `<p class="entry__card-meta entry__supervisor">${normalizeLocale(locale) === 'zh' ? '导师：' : 'Supervisor: '}<a href="${escapeHtml(item.supervisor.url)}" target="_blank" rel="noreferrer">${escapeHtml(textFor(item.supervisor.name, locale))}</a></p>`
      : '';
